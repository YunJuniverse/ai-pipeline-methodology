...
# 60_meta/ — Meta-Methodology

> **이 폴더는 본 저장소(methodology source)에만 존재합니다.**
> 외부 프로젝트에 `methodology init` / `sync` 로 주입되지 **않습니다**.
> 강제 메커니즘은 `50_tools/methodology.py` MANIFEST의 `excluded_paths`.
> 위상: [`00_foundation/WHITEPAPER.md`](../00_foundation/WHITEPAPER.md) §부록 C — 60 slot.
...
1. **외부 프로젝트들이 적용하는 방법론의 원천** (외부에 주입되는 자산: `10_guides/`, `40_resources/`, `50_tools/`, `.ai/`, `00_foundation/`, root 마크다운)
2. **본 저장소 자체를 운영·진화시키는 메타-방법론** (`60_meta/` — 본 폴더)
...
| 계층 | 메커니즘 |
|---|---|
| **1차 (whitelist)** | `methodology.py` MANIFEST는 *명시된 경로만* 복사. `60_meta/`가 어디에도 명시되지 않으면 *애초에* 복사 안 됨. |
| **2차 (excluded_paths)** | MANIFEST에 `excluded_paths: ["60_meta"]` 명시. sync/init 시 안전망 검증. 실수로 다른 곳에 추가해도 차단됨. |

검증 명령:
```bash
python3 50_tools/methodology.py manifest-check   # 안전망 검증 (구현 시점에 따라)
```
...
| 항목 | `40_resources/catalog/` (도메인) | `60_meta/catalog/` (메타) |
|---|---|---|
...
### 5.1 `rfc/`
- 백서·헌법·운영 규칙 변경 *제안*. 머지된 RFC는 `30_dev/adr/`에 ADR로 박힘.
- Class C 변경의 *논의 공간*.
...
- 형식: [`10_guides/03_AI_관찰_로그_작성_규칙.md`](../10_guides/03_AI_관찰_로그_작성_규칙.md) 와 동일. 단 *대상이 방법론 운영*.
- 예: "RFC 작성 시 부록 C 매번 검색 필요 → 검색 효율 개선 후보".

### 5.5 `catalog/` (메타-Catalog)
- 형식: [`40_resources/catalog/_README.md`](../40_resources/catalog/_README.md) 의 스키마를 그대로 따름. 단 `id: MC-NNN`.
...
| 실수 | 결과 | 교정 |
|---|---|---|
| 도메인 마찰을 `60_meta/catalog/`에 넣음 | 외부 프로젝트가 그 솔루션의 혜택 못 받음 | 판정 기준(§4) 재적용 → `40_resources/catalog/`로 이동 |
| 메타 마찰을 `40_resources/catalog/`에 넣음 | 외부 프로젝트에 무관한 솔루션이 주입됨 | 제거 후 `60_meta/catalog/`에 재등록 |
| `60_meta/`를 MANIFEST `shared_paths`/`init_paths`에 추가 | 주입 격리 깨짐 | `excluded_paths` 검증 fail → CI 차단 |
...
- [ ] `60_meta/`의 어떤 경로도 MANIFEST `shared_paths`/`init_paths`에 들어 있지 않은가
