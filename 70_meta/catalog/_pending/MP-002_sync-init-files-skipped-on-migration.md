...
새 버전에서 `init_files`에 `(50_resources/templates/context.json, .ai/context.json, True)` 같은 항목이 추가되어도, *기존 적용 프로젝트는 sync로 마이그레이션될 때* 이 파일을 받지 못한다. `sync`는 *shared_paths만 덮어쓰고 init_files는 init 단계 전용*이기 때문.
...
- 메타 관찰: `70_meta/observations/2026-05-12_v3.1-to-v3.2-migration.md` §F-002
- 적용 예: `migrations/v3.1_to_v3.2.py` `_materialize_l0`
- 코드: `60_tools/methodology.py` `cmd_sync`, MANIFEST
