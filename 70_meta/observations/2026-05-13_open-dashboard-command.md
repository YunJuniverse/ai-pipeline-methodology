...
friction:
  - id: F-001
    ...
    cost_minutes: 0
    resolution: "open-dashboard.command 신설 (macOS .command 파일, +x). Finder 더블클릭 시 Terminal 에서 자동 실행. 내용은 단순 'cd $(dirname) && python3 60_tools/methodology.py dashboard --open'. cmd_dashboard 에 --open 옵션 추가 (macOS: subprocess.Popen(['open', url]) / 기타: webbrowser.open). 적용 프로젝트에도 MANIFEST shared_paths 로 자동 전파."
    repeat_of: null
  - id: F-002
