...
_start/ 1차 빌드 후 사용자 요구로 구조 재편 — (mac)(windows)(linux) 파일명 + assets/ 하위 정리 + 루트 4 PNG 정리. clean_legacy() 가 옛 자산을 자동 제거해 멱등 빌드 보장. 격리 검증 통과 (적용 프로젝트 init 시 새 구조·실행권한·70_meta 격리 모두 ✅). F-003 (사용자 작업 산출물의 루트 클러터 정리) 는 *작업물 누적 패턴* 의 일반화 후보 — 향후 .png/.tmp 등 빌드 산출물 자동 정리 hook 검토 가능.
