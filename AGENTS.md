...
- **세션·작업 종료 절차 (의무, 모든 AI 모델 공통)**: 자연스러운 작업 단위가 끝나면 다음 4개 라이브 파일을 *AI가 자동으로* 갱신한 뒤 `python3 60_tools/methodology.py wrap` 으로 검증 → 결과를 사용자에게 보고. 사용자는 다음 turn에서 결과를 보고 수정 요청 가능.
  ...
  wrap 출력이 `4/4 ✓` 일 때만 종료. `✗` 가 있으면 누락 갱신 후 다시 호출. wrap v4.1+ 는 sha256 콘텐츠 해시 비교 — *실제 내용 갱신* 만 통과.
- **컨텍스트 컴팩션 경계 (의무, 긴 세션)**: 컨텍스트가 한계에 근접하거나 하네스가 요약(compaction)을 예고하면, *요약 전에* 라이브 파일(특히 `.ai/checkpoint.md`)을 먼저 갱신한다. compaction 은 "세션 중간 인계" — 파일에 상태가 있으면 요약이 잃어도 복원된다. 보존/폐기 규칙은 `20_guides/06_컨텍스트_컴팩션_프로토콜.md`. 핵심: *"compaction 후의 내가 이걸 잃으면 사용자에게 다시 물어야 하나?"* → 그렇다면 보존, 파일 재로드로 되는 것은 경로만 남기고 폐기.
- **자율 진행 예산·정지 (권장, 대규모 자율 작업)**: 사람 확인 없이 여러 단계를 자율 진행할 땐 착수 전 개략 예산(파일/PR/반복 규모)을 선언하고, no-progress 2회·반복 캡·예산 초과 시 멈춰 보고한다("예산 내 자율, 초과 시 보고"). 상세: `20_guides/07_자율진행_예산_및_정지조건.md`. 멀티에이전트 팬아웃 규약은 `20_guides/08_서브에이전트_오케스트레이션.md`.
