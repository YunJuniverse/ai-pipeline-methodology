...
- **머지 착지 (METH-133)**: push 후 `methodology.py land`(또는 `ship --land`)로 **PR 머지→기본 브랜치 동기화→maincheck**까지 한 번에. **자동 머지는 Class A + CI green 일 때만** — 변경 경로에 Class B/C 트리거(마이그레이션·인증·과금·외부 API 계약·백그라운드 작업·CI 파이프라인·법무/정책)가 걸리거나, CI 실패/진행 중이거나, 판정 자체가 불가하면 land 가 거부하고 사람을 부른다(fail-closed). `--no-ci-check` 는 무검증 머지라 기본 금지. methodology:ADR-004.
- **운영 모드 키워드 트리거 (의무, METH-136)**: 사용자 요청에 아래 키워드가 나오면 **해당 지침 본문을 먼저 로드한 뒤 착수**한다 — 이 요약만 보고 시작하지 않는다(요약에는 조건·정지 규칙이 없다). 키워드 표·경계 판정은 `20_guides/01` §5.11.
  - **실험 모드 · god 모드 · 자율 실험 · "제약 없이" · "프로토타입 자유롭게" · 샌드박스 · lab** → **지침 28**. 샌드박스 4조건(실데이터·실사용자·실결제·대외산출물 **없음**)을 **전수 확인**하고 하나라도 어긋나면 일반 모드로 진행하며 그 사실을 알린다. 통과 시 `Lab` 선언(기한·범위·목적 필수, 만료 시 자동 일반 모드)으로 Class B/C 를 **유예**하고, 경계를 넘어 운영으로 올릴 때 **졸업 게이트 7항**을 한 번에 정산. 게이트를 없애는 게 아니라 옮기는 것 — 시크릿·비가역 대외 행동·타인 자원·경계 확대 결정은 실험 모드에서도 그대로 막힌다.
  - **자율주행 · 오토파일럿 · "N시간 알아서" · "밤새 돌려" · 무인 진행 · 루프 작업** → **지침 29**. 사전 요구 4종(권한 사전 승인·샌드박스 경계·컴팩션 앵커·ground-truth 판정수단) 확인 — **권한·샌드박스가 없으면 착수하지 않는다.** 시간을 **사이클(≈45~90분)로 환산**해 기획을 먼저 파일로 남기고(`40_dev/snapshots/<date>_autopilot-plan.md`) 개발→검토→QA→신규작업 도출 루프. 진척은 **ground-truth(테스트·빌드·렌더·diff)로만** 판정(자기평가 금지). **기획이 소진되면 시간이 남아도 종료** — 시간은 할당량이 아니다.
  - 시간 지정 없는 일반 자율("알아서 끝까지"·대규모 전파) → **지침 07**(예산 선언 + 정지 조건). 29보다 가벼운 단발 자율.
...
- **외부 게이트 = Blocked 강제 (의무, METH-122)**: 대표 승인·결제·자격증명·타인 응답 등 *사람/외부만 풀 수 있는* 대기 항목은 TODO `## Blocked`로 이동하고 HANDOFF Blockers와 일치시킨다 — Ready 체크박스·notes 산문에 숨기지 않는다(전수조사: 4개 repo에서 Blocked 0건인데 실 병목 다수, 최장 19일 은닉). **Blocked 등록 전 기존 `## Blocked`·HANDOFF Blockers 를 grep 해 같은 건(PM 판정 포함)이 있는지 확인**하고, 월 1회 후속 결정과 대조해 stale 을 종결한다(METH-147). 새 TODO ID 는 `methodology.py reserve` 로 예약한다(병렬 세션 번호 충돌 방지, 지침 30 §7). 라이브 파일이 규정 2배를 넘으면 wrap --strict 가 fail — `methodology.py rotate --apply`로 회전(삭제 아님·아카이브 이관). dev 서버 실행 중 build 금지 — ship이 차단하며 수동 빌드는 `60_tools/build-guard.sh` 경유.
...
- **방법론 업데이트 제안 (캡슐, METH-117)**: 사용자가 "방법론에 반영해줘 / 나중에 방법론 업데이트에 넣어줘"라고 하면 → **의무**로 `methodology.py capsule --slug <s> --target <t> --summary "<요지>"` 캡슐 생성(1제안=1캡슐=1파일) 후 커밋·push까지. AI 자발 제안은 근거(재발·비용·실사례) 있을 때만 권장 — 노이즈 금지. **포인터+요약 원칙**(git SHA·PR·경로는 `--ref`로, 원문 덤프 금지·시크릿 금지). friction(막힌 사실)과 구분 — 마찰 파생 제안은 `--friction-ref`. 수거는 상류 `collect`(수동)·수거 여부는 상류 원장. 상세 `50_resources/meth_outbox/_README.md`.- **스택-PR 금지·Done 검증 (의무, METH-120)**: PR은 **main 직행 단일 PR**만 — 열린 PR 브랜치 위에 다음 작업을 쌓지 않는다(앞 PR 머지 후 main에서 새로 분기). 중간 브랜치로 머지되면 "머지됨"이어도 main엔 코드가 없다(6개 repo 실사고). **TODO Done 전이·배포 판정 전 `methodology.py maincheck <sha>`로 main 도달을 기계 확인** — 미도달 상태의 Done 표기는 허위 기록이다.
- **컨텍스트 컴팩션 (의무, 긴 세션)**: 요약 예고 시 *요약 전에* 라이브 파일(특히 `.ai/checkpoint.md`) 먼저 갱신 — 파일에 있으면 요약이 잃어도 복원. 규칙 `20_guides/06_컨텍스트_컴팩션_프로토콜.md`.
