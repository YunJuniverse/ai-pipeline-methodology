...
- **Working on**: **METH-100 v3.2 backward-compat 코드 정리**. 현존 repo 7곳 전부 v4.0이라 dead였던 v3.2 폴백 제거. **methodology.py**: `_LAYOUT_V32`·`methodology_layout()` 구조탐지 삭제→v4.0 고정, `_observation_dir`/`_wrap_obs_dirs`를 50_resources/70_meta 하드코딩(40_resources/60_meta 폴백 제거). **generate-dashboard.py**: `_LAYOUT_V32`·`dash_layout` 탐지 삭제→v4.0 고정, `_count_observations`·`assemble`의 docs//40_resources/60_meta/legacy-root 폴백 제거(50_resources/templates 유효 폴백만 유지), 푸터 "v3.2"→"v4.0" 라벨·stale 도크스트링 수정. **경계**: `migrations/v3.2_to_v4.0.py`(이관 스크립트=유일 escape hatch)·런처/훅 3-tier 탐지(부트스트랩 tolerance)는 의도적 보존. py_compile·dashboard 재생성(obs 카운트 양쪽 dir 정상)·wrap 검증. Class A. #99 위 스택 내용(096~099 포함) → base=main 단일 PR. PR 대기.
...
- Current PR: METH-100 v3.2 compat 정리 (신규, base=main, 096~099 포함) · #88(099+096~098 복구)=OPEN · 095=#84 머지 · 063~094 = #53~#83 머지 완료
- Current issue:
- Relevant ADRs:
