...
- **Working on**: **METH-136 운영 모드 키워드 트리거**(2026-08-07) — 지침 01 §5.11 라우팅 표 + CLAUDE/AGENTS 를 동작 지시로 전환. 지침 28·29 가 *선언만 되고 로딩 경로가 없던* 갭을 닫음. branch `feat/mode-keyword-routing`. 직전: METH-133/134/135 전파 종결 12/12(#141).
...
- 2026-08-07: **METH-136 운영 모드 키워드 트리거 (Class A)** — 지침 28·29 를 만들었지만 *키워드로 불러오는 경로*가 없던 갭을 닫음. 지침 01 §5.11 운영 모드 라우팅 표(실험/자율주행/07/land + 경계 판정 3항) 신설, CLAUDE.md·AGENTS.md §2 를 서술→**동작 지시**로 전환("본문 먼저 로드 후 착수, 요약만 보고 시작 금지"). 안전장치: 속도 요구만으로 실험 모드가 켜지지 않고 **샌드박스 4조건 확인이 선행**.
