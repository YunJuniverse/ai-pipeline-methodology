...
- **Working on**: **METH-120+121 구현 완료** (2026-07-29) — `maincheck`(main 도달 검증·스택-PR 사고 방지) + observe 스키마 강제(repeat_of enum·메타 자동 채움·domain 필수·상용구 제거·품질 경고). tests 11종+기존 회귀 통과. branch `feat/meth-120-121-guards`, PR 대기. 머지 후 sync-all 전파 필요(methodology.py·CLAUDE/AGENTS shared). 직전: METH-119 트리아지 종결(#120 머지)·insta-toon 복구(그 repo #7 머지).
