...
- **Working on**: METH-084 **skeleton 서브시스템 활성화 + 죽은 필드 정리**. 사용자 "skeleton 필요한가?" → 판정: AI-LOG와 달리 **고유 기능(catalog→skeleton→주입 환류 루프)·자기완결·실체 있음 → 유지**. 단 저활용 상태라 **활성화**(사용자 승인): ① 파이프라인 end-to-end 검증(init→build→apply, frontend-design-tokens 9파일+lock 주입 정상) ② 발견 버그 정리 — `bakes-in.json.last_built`는 아무도 갱신·참조 안 하는 죽은 필드(실제 시각은 lock `built_at`), AI-LOG와 동종 → CLI init·양 bakes-in·_README에서 제거(SSOT). 양 도메인 lock 재빌드. 후속: 레슨 만날 때마다 catalog 엔트리 추가(현재 active 1개 C-001). 내부 정합성. Class A. PR 대기.
- **Current mode**: fullstack
- **Next TODO**: ① **심화·정합·활성화 사이클 완료** — 템플릿(063~071)+지침군(073~078)+오케(079)+마스터플랜(080)+prompts(081)+운영원칙(082)+메타파일(083)+skeleton(084). 남은 후보: agency/ops 템플릿(proposal·qa·operation·profitability·execution-plan·wbs), 메타/dev 지침(02~09·19~20). ② **누적 심화분(073~084) 다운스트림 sync** — gamblescan·icons(072까지 반영됨→073~ 추가) + 홀드 3곳 clean 후. ③ **graph.json 노드 완성**(guide 02~09·19~21 누락, 별건). ④ skeleton 후속: 레슨→catalog 엔트리 축적. **프로세스: branch-first 준수.**
- **Blockers**: none

## Active Links

- Current PR: METH-084 skeleton 활성화 (신규) · METH-083 메타 파일 = #72 머지 완료 · 063~082 = #53~#71 머지 완료
- Current issue:
- Relevant ADRs:
