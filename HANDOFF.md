...
- **Working on**: METH-102 — 라이브파일 경계 재분리(b: HANDOFF=상태보드 / checkpoint=서사) + 상시 브리프 `00_briefs/standing/`(반복작업 SOP, boot이 항상 노출) 신설. 상세 서사는 checkpoint. Class A(7 repo). PR base=main 대기.
...
- Current PR: METH-102 경계재분리+상시브리프 (신규, base=main, #90 boot 포함) · ⚠️ #90 OPEN(미머지, gh 확인 필요) · 095~100 = #84·#89 머지 · 063~094 = #53~#83 머지
- Current issue:
- Relevant ADRs:
...
- 2026-07-10: **METH-102 라이브파일 경계 재분리 + 상시 브리프 (Class A, PR base=main 대기)** — HANDOFF=상태보드/checkpoint=서사 경계를 §4·§2·템플릿·에 못박아 중복 제거(checkpoint의 "미해결 결정사항"=HANDOFF Open Issues 중복 제거). `00_briefs/standing/`(반복작업 SOP·아카이브 안 됨) 신설, boot이 ★로 항상 최상단 노출. 반복 작업이 새 세션에 안 잊히게. #90(boot) 포함 base=main.
