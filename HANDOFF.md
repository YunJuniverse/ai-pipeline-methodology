...
- **Working on**: METH-081 **prompts/ 층 전면 현행화**. 사용자 질문(운영원칙·prompts 역할)에서 프롬프트층이 심하게 drift 발견 → 현행화. 구모델(`briefs/`·`snapshots/plans/vN`·"항상 6종") → 현모델(`00_briefs/current/`·`30_planning/` 라이브·모드 선택). **목차 복제 제거**(구조 SSOT=지침, METH-080과 동일 교정). 기획서 프롬프트 6종 재작성 + **ai-feature(16)·eval-guardrail(17) 신설**(8종 커버) + 코드 스냅샷 4종 역할 명확화 + dev-spec 현행화 + **`_README.md` 신설**(프롬프트↔지침↔템플릿↔모드 매핑). 상위 문서(README·50_resources/_README) 정정. 내부 정합성(리서치 없음). Class A. PR 대기.
- **Current mode**: fullstack
- **Next TODO**: ① **문서별 심화 프로그램 대부분 완료** — 템플릿 13종(063~071)+지침군 6종(073~078)+오케스트레이션(079)+마스터플랜 SSOT(080)+prompts 층(081). 남은 후보: agency/ops 템플릿(proposal·qa·operation·profitability·execution-plan·wbs), 메타/dev 지침(00·02~09·19~20). ② **누적 심화분(073~081) 다운스트림 sync** — gamblescan·icons(072까지 반영됨→073~ 추가) + 홀드 3곳 clean 후. ③ **graph.json 노드 완성**(guide 02~09·19~21 누락, 별건). **프로세스: branch-first 준수.**
- **Blockers**: none

## Active Links

- Current PR: METH-081 prompts 층 현행화 (신규) · METH-080 마스터플랜 SSOT = #69 머지 완료 · 063~079 = #53~#68 머지 완료
- Current issue:
- Relevant ADRs:
