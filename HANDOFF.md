...
- **Working on**: setup-*.{ps1,sh} 두 파일을 _start/settings/ 하위로 이동 — 진입점(3) + README + assets/ + settings/ + .cache/ 구조로 시각적 위계 명확화. 스크립트 내부 경로(`$root`/`$ROOT`)·clean_legacy·README 모두 갱신.
- **Current mode**: fullstack
- **Next TODO**: 3개 적용 프로젝트 sync 전파, 루트 README.md in-spire 리브랜딩, METH-020 (MC-002 N=6 승급)
- **Blockers**: none
