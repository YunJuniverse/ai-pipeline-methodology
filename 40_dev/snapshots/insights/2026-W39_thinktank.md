...
- 케이던스: 주당 약 9.4건
- task_type 분포: docs 96, feature 32, bugfix 24, refactor 20, research 6, bootstrap 1
- 마찰 총계: 47건 · Catalog 재적중(repeat_of): 9건 · 승급 후보(≥2회): 3건
...
- `watch` x1: 20_guides/README.md 변경이력 순서
...
- `watch` x1: 그래프 레이아웃
- `watch` x1: 다운스트림 pre-push 훅 sync 면제 패턴
...
- `watch` x1: 워크트리 push 후 로컬 기본브랜치 미갱신
- `watch` x1: 지침 26 FLUX 라이선스 문장
- `watch` x1: 판정 근거를 측정 없이 단정
- `watch` x1: 한글 파일명·본문 편집을 perl/sed+hex로 하니 개행 삭제·인코딩 mojibake 발생(doc_id·title 병합)
- `watch` x1: 훅 sync 경로 판정
- `watch` x1: 훅 sync 경로 판정 한글 경로
- `watch` x1: 훅이 wrap 산출물로 repo 를 dirty 로 만듦

## 마찰 비용 회고 (Retro)

- 마찰 **47건 · 711분** · 재발(repeat_of) 9건 101분 (**14%**)
- phase 기입률: **0/47** — 기입률이 낮으면 아래 분포는 표본 편향이다
...
- 60분 · `ship push step` — 2026-07-24_ship-push-verify
- 40분 · `다운스트림(icons-invest)에 축적된 대량 도메인 작업을 상류 방법론으로 역환류할 때 원료가 스냅샷·빌드스크립트·관찰로그 3곳에 흩어져 통독 비용 큼` — 2026-07-25_ir-deck-methodology-guide-22
- 30분 · `새 세션이 부팅 계약(브리프 로드·dashboard)을 건너뛰고 IR 질문에 바로 뛰어들어 기존 프로세스 모른 채 오답 — 부팅 강제장치 없음 + HANDOFF 81KB 비대화로 부팅 프라이머 무력` — 2026-07-10_meth-101-boot-cmd-size-lint
- 30분 · `그래프 레이아웃` · 재발 — 2026-07-15_graph-viz-dagre
- 30분 · `60_tools/methodology.py:1604` — 2026-09-01_land-plan-union-refine
...
- `2026-09-02_capsule-triage-round4-draft` — domain `meta`, task `docs`
- `2026-09-02_capsule-triage-round4-reflect` — domain `meta`, task `feature`
- `2026-09-02_meth-142-closeout` — domain `meta`, task `docs`
- `2026-09-02_meth-142-propagation` — domain `meta`, task `docs`
- `2026-09-02_meth-142-remaining-decisions` — domain `meta`, task `feature`
- `2026-09-02_meth-142-second-propagation` — domain `meta`, task `docs`
- `2026-09-02_meth-143-propagation` — domain `meta`, task `docs`
- `2026-09-02_meth-143-wrap-structure-check` — domain `meta`, task `feature`
- `2026-09-02_meth-144-145-propagation` — domain `meta`, task `docs`
- `2026-09-02_meth-144-followups` — domain `meta`, task `docs`
- `2026-09-02_meth-145-hook-quotepath` — domain `meta`, task `bugfix`
- `2026-09-02_meth-146-hook-wrap-readonly` — domain `meta`, task `bugfix`
- `2026-09-02_meth-146-propagation` — domain `meta`, task `docs`
- `2026-09-02_session-close-rotate` — domain `meta`, task `docs`
- `2026-09-14_capsule-collect-round5` — domain `meta`, task `docs`
- `2026-09-14_capsule-triage-round5-draft` — domain `meta`, task `docs`
- `2026-09-15_hook-shared-paths-managed-files` — domain `meta`, task `bugfix`
- `2026-09-15_meth-147-closeout` — domain `meta`, task `docs`
- `2026-09-15_meth-147-guides-catalog` — domain `meta`, task `docs`
- `2026-09-15_meth-147-inbox-cleanup` — domain `meta`, task `docs`
- `2026-09-15_meth-147-tools-bundle` — domain `meta`, task `feature`
- `2026-09-15_readme-changelog-order-repair` — domain `meta`, task `bugfix`
- `2026-09-18_capsule-collect-round6` — domain `meta`, task `docs`
- `2026-09-18_capsule-triage-round6-draft` — domain `meta`, task `docs`

## Collected Capsules (_inbox)

- `DUP-TARGET` x4: target `catalog` — repo cafe24-renewal
- `single` x1: target `guide-26` — repo ai-icons
- `single` x1: target `guide-27` — repo ai-icons
- `single` x1: target `guides` — repo icons
- `single` x1: target `skeleton/hosted-platform-customization` — repo cafe24-renewal
- `single` x1: target `tool/observe` — repo cafe24-renewal
- `single` x1: target `tool/thinktank` — repo cafe24-renewal
