...
- **올릴 것은 «다른 곳에서도 참인 규율»뿐이다** (METH-142). 플랫폼·벤더 고유 사실(특정 SaaS 의 렌더 동작·캐시 계층·API 기벽)은 **repo 로컬 정본**(`20_constraints/` 류)에 두고 발신하지 않는다. 상류 catalog 는 «재발한 문제 1건당 1엔트리»의 *교차 프로젝트* 자산이라, 한 플랫폼 안에서만 재사용되는 지식은 상류에서 검색되지도 재사용되지도 않으면서 트리아지 비용만 만든다. 판별 질문: **다른 플랫폼·다른 repo 에서도 이 문장이 참인가?** 참이면 캡슐, 아니면 로컬 정본. 고유 사실에서 *뽑아낸 일반 규율*(예: 「판정 오라클은 캐시를 공유하지 않는 전용 표면에 둔다」)은 캡슐로 올린다 — 사실이 아니라 규율이 상류의 재료다.
