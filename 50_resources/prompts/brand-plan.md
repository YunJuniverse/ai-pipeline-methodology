...
- Read `20_guides/14_브랜드기획서_작성_지침.md` — follow its structure exactly.
- Read `40_dev/snapshots/plans/service/` and `40_dev/snapshots/plans/marketing/` (최신).
- Read prior versions in `40_dev/snapshots/plans/brand/` if they exist.
- Add frontmatter block and snapshot warning header.
- Write to `40_dev/snapshots/plans/brand/v{N}-{YYYY-MM-DD}.md`.
- Update `HANDOFF.md`.
