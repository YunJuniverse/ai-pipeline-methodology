...
- Read `20_guides/12_운영기획서_작성_지침.md` — follow its structure exactly.
- Read `40_dev/snapshots/plans/service/` (최신) — 서비스 흐름과 정합성 유지.
- Read prior versions in `40_dev/snapshots/plans/ops/` if they exist.
- Add frontmatter block and snapshot warning header.
- Write to `40_dev/snapshots/plans/ops/v{N}-{YYYY-MM-DD}.md`.
- Update `HANDOFF.md`.
