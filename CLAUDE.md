...
- **실험 모드 (지침 28)**: 프로토타입은 샌드박스 4조건(실데이터·실사용자·실결제·대외산출물 **없음**)을 전부 만족할 때 `Lab` 선언으로 Class B/C 를 **유예**한다 — 기한·범위·목적 명시 필수, 만료 시 자동 일반 모드. 경계를 넘어 운영으로 올릴 때 **졸업 게이트 7항**을 한 번에 정산. 게이트를 없애는 게 아니라 옮기는 것 — 시크릿·비가역 대외 행동·타인 자원·경계 확대 결정은 실험 모드에서도 그대로 막힌다.
- **자율주행 (지침 29)**: 장시간 자율 턴은 시간을 **사이클(≈45~90분)로 환산**해 기획을 먼저 파일로 남기고(`40_dev/snapshots/<date>_autopilot-plan.md`), 개발→검토→QA→신규작업 도출 루프를 돈다. 진척은 **ground-truth(테스트·빌드·렌더·diff)로만** 판정 — 자기평가 금지. **기획이 소진되면 시간이 남아도 종료**(시간은 할당량이 아님). 착수 전 권한 사전 승인·샌드박스 경계가 없으면 시작하지 않는다.
